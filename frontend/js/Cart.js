...
function showCart() {
    ...
    if (currentCart && currentCart.length >= 1) {
        ...
        `
            <tr class="text-center">
                <td>${title}</td>
                <td class="max-width-50 text-start text-sm-center ps-0 ps-md-2">
        ...
        div.innerHTML += `
            <button class="btn shadow rounded-pill mt-5 p-2 px-3 cartBtnHover" id="delete-cart">Vider le panier</button>
            <p class="fs-3 mt-3">
        ...
        const linksToDeleteCart = document.querySelector("#delete-cart");
        linksToDeleteCart.addEventListener('click', function() {
            if (confirm('Souhaitez-vous supprimer la totalité de votre panier ?')) {
                localStorage.removeItem("Panier");
                numberOfItemsInCart();
                mainDiv.removeChild(cartBody);
                showCart();
